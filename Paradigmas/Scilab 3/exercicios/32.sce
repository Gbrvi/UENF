...
deff('y=myf(A,B,C,D,x)',['y=A*x^2+B*x^2+C*x+D']);  // função de terceiro grau
deff('dy=dyf(A,B,C,D,x)',['dy=2*A*x+2*B+C']); // função derivada

h0=gcf();
h0.figure_size= [500,700];
h0.figure_position = [40,70];
h0.figure_name="UENF - Valores dos coeficientes de "+"Ax2+Bx+Cx+D";
