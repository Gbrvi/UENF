...
#include <stdlib.h>
#include <string.h>
#include "AVLTreeString.h" //inclui os protótipos
...
int consulta_ArvAVL(ArvAVL *raiz, char* valor){
    ...
    while(atual != NULL){
        ...
        else if(strcmp(atual->info, valor) < 0)
            atual = atual->dir;
        else
            atual = atual->esq;
    }
...
int insere_ArvAVL(ArvAVL *raiz, char* valor){
    ...
    printf(" %s ", atual->info);
    printf(" %d", strcmp(valor, atual->info));

    if(strcmp(valor, atual->info) < 0){
            printf("\nt2.1");
        if((res = insere_ArvAVL(&(atual->esq), valor)) == 1){
            if(fatorBalanceamento_NO(atual) >= 2){
                if(strcmp(valor, (*raiz)->dir->info) < 0){
                    RotacaoLL(raiz);
                }else{
                    RotacaoLR(raiz);
                }
    ...
    }else if(strcmp(valor, atual->info) > 0){
        printf("\nt2.2");
